...
## 5. 단어 세팅 팝업 (오후, 실제 화면 보며) — `c5ebbc1` `e65269f`

- **잠금이 안 보임** → 호박색 「시험지 잠김 — 채점 N개」 배너, 저장 버튼 자리 🔒, 체크·칩·랜덤·비율·미리보기 편집 전부 비활성.
- **난잡함** → 관리 액션(뜻 보완/예문 생성/다시 올리기) ghost 강등, 비율 패널 접힘(한 줄 요약), 단어 행 한 줄+가능한 칩만, 7종 카운트 한 곳.
- **뜻 보완/예문 생성 활성 조건** — 채울 게 있을 때만 + 개수 배지. 뜻 보완은 단어장 전체 대상으로 변경(전엔 시험에 고른 항목만). 상태줄 「예문 N/M」.
- **"예문 생성 안 했는데 예문 유형이 선택돼"** — DB 확인: 25주차는 11:43에 실제로 예문 생성 API 가 돌았음(`vocab_examples_generated_at`). 26주차(8/28, 예문 0)에서 진짜 문제는 **비율 패널이 예문 % 를 받아 놓고 랜덤 때 조용히 원본으로 채우던 것** → [applySourceAvailability](../../src/lib/vocab-test-ratio.ts): 후보 없는 유형 0으로 접고 재분배, 요약줄 「반의어·예문뜻 제외」 배지, 패널 입력 비활성 + '예문 생성 필요'.
- **선택형 오답이 전부 같은 단어** — 반의어 없으면 폴백이 항상 단어장 1번(`participate`). [vocab-choice-distractor.ts](../../src/lib/vocab-choice-distractor.ts): 같은 단어장에서 같은 품사·비슷한 생김새 우선, 유의어/파생어/반의어/예문 속 단어 제외, 단어별 고정 해시. **AI 안 씀. 반의어 필수 아님.**
- **문항 번호가 파트를 거슬러 올라감**(A 뜻쓰기 15~30, B 8~14…) — 랜덤이 귀한 유형부터 채워 그 순서로 번호를 매긴 것. `sortIdsBySection` 으로 번호 = 인쇄 파트 순(뜻쓰기→예문뜻→빈칸→선택). 잠긴 시험지는 저장 순서 유지. **이 규칙 전에 저장한 시험지는 한 번 재저장해야 번호가 새로 매겨짐.**

부작용: 뜻 보완 범위 확대(단어장 전체) → LLM 호출 늘 수 있음(서버가 빈 것만 + 캐시). 선택형 오답이 바뀌므로 **새로 저장하는 시험지부터** 오답이 달라짐(저장본은 prompt_text 고정).
