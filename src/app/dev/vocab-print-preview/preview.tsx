...
/**
 * embedded=true 면 개발자 도구(/dev)의 탭 안에서 렌더된다 —
 * 전체 화면 점유(min-h-screen)와 body 배경 강제를 끄고, URL 파라미터도 읽지 않는다.
 * embedded=false 는 단독 라우트(/dev/vocab-print-preview) — headless 캡처(?bare=1)가 이쪽을 쓴다.
 */
export default function VocabPrintPreview({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={bare ? 'bg-white' : `${embedded ? 'rounded-xl' : 'min-h-screen'} bg-gray-100 py-6 print:bg-white print:py-0`}>
