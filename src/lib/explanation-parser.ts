...
function extractIntent(block: string): string {
  ...
  if (m1) return m1[1]
  // 괄호 없는 형식: "출제 의도 내용[ ]" 또는 "출제 의도 내용 해석" / "출제 의도 내용 [해석]"
  // 다음 섹션이 괄호로 시작하면 여는 괄호까지 같이 끊어야 intent 끝에 '['가 섞이지 않는다.
  const m2 = block.match(/출제\s*의도\s+(.+?)(?:\s*\[\s*\]|\s*[[【]?\s*(?:해석|풀이|Words)|$)/)
  return m2 ? m2[1] : ''
}
