...
1. multi_select
   ...
   확인: 발문에 "모두 고르시오" / "옳은 것을 모두" / "있는 것을 모두" / "n개 고르(시오|면)" 포함
   ⚠️ 정답이 ①~⑤ 중 단일 번호이면 절대 multi_select 아님 → STEP 5 objective
       (발문에 "모두" / "골라 짝지은" 등 표현이 있어도 정답이 단일 번호면 단일 객관식)
   ⚠️ 예외: 발문에 "어법상" / "문법적으로" / "어법에 맞지 않는" 등 어법/문법 키워드가 있는
...
② 조합 선택형 (combination choice) — 분리 금지 + multi_select 금지
   조건 (둘 중 하나라도 해당):
   (a) [표 형식] 발문에 "(A), (B)에 들어갈 말" 등이 있고, 선택지 ①~⑤ 각 행에 (A)(B)(C) 값이 나열되어 학생이 번호 하나만 선택
       예1)
            (A)          (B)           (C)
       ①  enjoyable   discourage    accurately
       ②  unpleasant  recommend     accurately
       예2) 발문: "빈칸 (A), (B)에 들어갈 말로 가장 적절한 것은?"
            선택지:
            ①  (A) language      (B) cooperation
            ②  (A) self-esteem   (B) competition
            ③  (A) confidence    (B) punishment
            ④  (A) morality      (B) instruction
            ⑤  (A) morality      (B) instinct
       → 학생은 ①~⑤ 중 하나만 선택 → sub_label = null, correct_answer = 4
       ⚠️ 이 유형에서 (A)(B)는 빈칸 위치 표시일 뿐, 독립 채점 대상이 아님
          절대 sub_label "a","b"로 분리하면 안 됨
   (b) [보기 묶음형] 본문/발문 부근에 ⓐ~ⓕ 등 보기가 나열되고, 선택지 ①~⑤가 그 보기들의 조합
       예) 발문: "내용과 일치하는 문장을 [보기]에서 모두 고른 것은?"
                "옳지 않은 것만을 <보기>에서 모두 고른 것은?"
                "어법상 틀린 것만 골라 짝지은 것은?"
           보기: ⓐ … ⓑ … ⓒ … ⓓ … ⓔ … ⓕ …
           선택지: ① ⓐⓔ  ② ⓐⓒⓕ  ③ ⓑⓓ  ④ ⓑⓒⓓ  ⑤ ⓔⓕ
   판정 기준 (공통): 학생이 최종적으로 ①~⑤ 중 하나만 선택하는 형식
   → 무조건 단일 objective (sub_label = null, correct_answer = ①~⑤ 정답 번호 1개,
                          correct_answer_text = null)
   ⚠️ 발문에 "모두" / "모두 고른" / "골라 짝지은" 등이 있어도 multi_select 아님
   ⚠️ 보기 기호(ⓐⓒⓕ 또는 a,c,f)를 correct_answer_text에 저장 금지 — 정답은 ①~⑤ 번호만
   ⚠️ 어법 "n개 고르" 형(⑥번)과 구분: ⑥번은 학생이 ①~⑤ 중 N개를 고르는 형식,
       이 ②번은 학생이 ①~⑤ 중 1개만 고르는 형식
...
⑥ 어법 "n개 고르" 형 객관식
   조건 (모두 충족):
   - 발문에 "어법상" / "문법적으로" / "어법에 맞지 않는" 등 어법/문법 키워드 포함
   - 발문에 "n개 고르시오" / "n개 고르면?" 등 복수 선택 지시
   - 정답이 선택지 번호 N개 (예: ②④)
   처리:
   - 정답 번호 작은 것부터 sub_label a, b, c... 순으로 분리
   - 각각 question_style = "objective", correct_answer = 해당 번호 1개, correct_answer_text = null
   - 각 sub_label에 서로 다른 문법 태그(question_type) 부여 가능
   이유: 각 선택지가 서로 다른 문법 포인트(분사구문/수동태/관계사 등)를 묻기 때문 — 오답분석에서 유형 구분 필수
