...
Step 0 [단어·구 단답형 조기 정답 판정 — 정답 확정 전용]:
  모범답안이 단어 또는 짧은 구(3단어 이하)인 경우:
  - 학생 답안에서 기호(e:, (e), ⓔ 등)·화살표(→)·원래 표현을 제거한 뒤 공백·대소문자 무시하고 비교
  - 일치하면 즉시 is_correct: true (Step 1~4 생략)
  - 불일치하더라도 즉시 false로 판정하지 말 것 → Step 1로 내려가서 계속 판단
  ※ Step 0는 확실한 정답을 빠르게 처리하는 것이 목적. 오답 판정 수단이 아님.
  예) 모범답안 "watching" / 학생 "watching"        → 즉시 true
  예) 모범답안 "watching" / 학생 "e: watching"     → 기호 제거 후 "watching" 일치 → 즉시 true
  예) 모범답안 "watching" / 학생 "watched→watching" → 화살표 뒤 "watching" 일치 → 즉시 true
  예) 모범답안 "had"      / 학생 "have"             → 불일치 → Step 1로 이동 (즉시 false 아님)
...
- '기호를 쓰고 고치시오' 형 (correct_answer_text가 "기호:수정어" 형식인 경우):
  학생 답안에서 기호(예: e, (e), ⓔ), 콜론, 화살표(→), 원래 표현 등을 제거하고 수정어만 추출하여 비교
  예) 모범답안 "e:watching" / 학생 답안 "e: watching"    → 정답
  예) 모범답안 "e:watching" / 학생 답안 "watched → watching" → 정답
  예) 모범답안 "e:watching" / 학생 답안 "watching"         → 정답
  같은 문항 여러 sub_label은 집합으로 비교 (순서 무관):
  예) 모범답안 ["c:asked", "e:watching"] / 학생 ["e:watching", "c:asked"] → 모두 정답
...
[분리 O]
① (A)(B)(C)(D) 등 복수 빈칸 — 단, 조합 선택형은 제외 (아래 ⑤ 참조)
   한 줄 나열이어도 반드시 분리
...
- 숙제 미제출(homework_done=0 또는 null): 반드시 한 줄 언급. 예: "이번 주 과제가 제출되지 않았습니다. 가정에서 한 번 확인 부탁드립니다."
- 단어 50% 이하 + 숙제 미제출 동시 해당: 클리닉 안내 추가. 예: "다음 수업 전 일찍 오셔서 클리닉 진행이 필요할 것 같습니다."
